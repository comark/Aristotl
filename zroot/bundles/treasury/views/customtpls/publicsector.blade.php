...
@if (TreasuryL\ContentHelper::get_custom('publicsector'))

    @foreach ( TreasuryL\ContentHelper::get_custom('publicsector') as $data_big_key=>$data_big_val )
        ...
        @if(isset($data_big_val) && $data_big_key == 'data')
            @foreach( $data_big_val as $data)
              {{ TreasuryL\ContentHelper::single_custom('publicsector',$data) }}
            @endforeach
        @endif
