...
@if (AristotlL\ContentHelper::get_custom('policies'))

    @foreach ( AristotlL\ContentHelper::get_custom('policies') as $data_big_key=>$data_big_val )
        ...
        @if(isset($data_big_val) && $data_big_key == 'data')
            @foreach( $data_big_val as $data)
              {{ AristotlL\ContentHelper::single_custom('policies',$data) }}
            @endforeach
        @endif
